...
| FAIL | 1 | 0 |
| WARN | 4 | 5 |
| Latencia promedio | 6551ms | 1121ms |
| Latencia máxima | 301632ms | 28668ms |
...
| 24 | `generá propuesta para pierrs` | ❌ | ⚠️ | network: fetch failed / latencia 28668ms > 5s |
...
| 50 | `carvi entregó de dafne → sí confirmo (2 pasos)` | ⚠️ | ⚠️ | paso1 200 action=consulta; paso2 200 action=pregunta; confirmación no ejecutó (pendiente/otra acción) / paso1 200 action |

## FAILs críticos

- **#24 [Local]** VENTAS: network: fetch failed
  - Respuesta (recorte): `fetch failed`
...
1. Revisar los FAILs listados: alinear prompts, validaciones o IntentRouter con el comportamiento esperado.
2. Investigar divergencias Local vs Prod (versiones, secretos, cold start, rate limit Odoo/Groq).
3. Muchos WARN: afinar heurísticas de QA o suavizar criterios de negocio documentados.
4. Mantener pruebas #49 (no regresión recepción vs pago) y #50 (confirmación) en CI.
5. Monitorizar latencia >5s (Groq/Odoo) y reintentos/backoff en carga de OCs.
